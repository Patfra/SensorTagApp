...
void *timed_loop(void* dane){ //argument is the array of SensorTag structures
//    clock_t time = clock();
...
void create_threads(int amount, SensorTag* strukturki, queue <ST_Data> *kju, gatt_connection_t ** conns){
    ...
    for(int i=0;i< amount;i++){ //Creating a vector of threads
        pthread_t threaddy;
        threadz.push_back(threaddy);
    }

    for(int j=0;j< amount;j ++){ //Creating Argument array
    ...
    for(int k=0;k<amount;k++){ //Enabling threads
        pthread_create(&threadz[k], nullptr,timed_loop,&argumenty[k]);
    }

    //Thread for saving data to CSV
    auto wd = Write_data();
    wd.kolejka = kju;
    ...
    //Joining all threads (ammount +1, because on additional thread for saving data to CSV)
